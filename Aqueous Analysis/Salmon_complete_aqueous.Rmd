...
food <- as.data.frame(read.table("./AQ/merged_file.txt",check.names=F, header = T))

diff_in_health_outcome <- read.table(file="./AQ/health_outcome.txt",header=T)
complete_health_outcome <- read.table(file="./AQ/complete_health_outcome.txt",header=T)
...
write.csv(food, "./AQ/food_table/Original_Dataset.csv", row.names=F)
```

Assigning Metabolite Names Such as: "c.1", "c.2", ... to Unique to Salmon Metabolites
```{r}
metabolite_info <- as.data.frame(read.table("./AQ/met_info.txt",header=T))

Metabolite <- paste("c.",1:nrow(metabolite_info),"_aq", sep = "")
...
write.csv(metabolite_info, "./AQ/food_table/Metabolite_Information.csv", row.names=F)
...
write.csv(missing_summary, "./AQ/food_table/Summary_Missing_less_75_Percent.csv", row.names=F)
```
...
for(i in 1:length(Metabolite)){
  ...
  ggsave2(filename=paste0("Metabolite_", Metabolite[i],".jpeg", sep=""), plot=p.out, device="jpeg",
          path=paste0("./AQ/Normalization_plot"))
}
```
...
write.csv(cross_section_ranked, "./AQ/food_table/Ranking_metabolites.csv", row.names=F)
```
...
write.csv(difference_ranked_change_analysis,
          "./AQ/food_table/Change_in_Ranks_Metabolites.csv", row.names=F)
```
...
write.csv(change_analysis_change_no_cov,
          "./AQ/food_table/Change_in_Ranks_Primary_Analysis.csv", row.names=F)
...
cross_section_out_period[grepl("c.",cross_section_out_period$test_type),"test_type"] <- "Metabolite"
...
write.csv(cross_section_out_period,
          "./AQ/food_table/Cross_section_analysis_Health_Metabolites.csv", row.names=F)
```
...
cross_section_out_pre[grepl("c.",cross_section_out_pre$test_type),"test_type"] <- "Metabolite"
...
```{r}
## Reshaping From Long to Wide
cross_section_out_pre <- reshape(cross_section_out_pre,timevar="test_type",idvar=c("Metabolite","Out_type","type"),direction="wide")
...
str(cross_section_out_pre_tile)
...
cross_section_out_post[grepl("c.",cross_section_out_post$test_type),"test_type"] <- "Metabolite"
...
## Reshaping From Long to Wide
cross_section_out_post <- reshape(cross_section_out_post,timevar="test_type",idvar=c("Metabolite","Out_type","type"),direction="wide")
...
out_per_1[grepl("c.",out_per_1$test_type),"test_type"] <- "Metabolite"
...
```{r, fig.height=15, fig.width=10, message=FALSE, ERROR = TRUE}
...
out_per_2[grepl("c.",out_per_2$test_type),"test_type"] <- "Metabolite"
...
out_per_3[grepl("c.",out_per_3$test_type),"test_type"] <- "Metabolite"
...
## Reshaping From Long to Wide
out_per_3 <- reshape(out_per_3,timevar="test_type",idvar=c("Metabolite","Out_type","type"),direction="wide")
...
out_per_4[grepl("c.",out_per_4$test_type),"test_type"] <- "Metabolite"
...
## Reshaping From Long to Wide
out_per_4 <- reshape(out_per_4,timevar="test_type",idvar=c("Metabolite","type","Out_type"),direction="wide")
...
change_analysis_out[grepl("c.",change_analysis_out$test_type),"test_type"] <- "Metabolite"
...
change_analysis_out <- reshape(change_analysis_out,timevar="test_type",idvar=c("Metabolite","Out_type","type"),direction="wide")
...
write.csv(change_analysis_out,
          "./AQ/food_table/change_in_Health_Metabolites.csv", row.names=F)
```
